...
function M.make_azure_openai_spec_curl_args(opts, prompt, system_prompt)
	...
	local deployment_id = opts.deployment_id
	local api_key = opts.api_key_name and get_api_key(opts.api_key_name)
	local api_version = opts.api_version or "2024-06-01"
	local url =
		string.format("%s/openai/deployments/%s/chat/completions?api-version=%s", endpoint, deployment_id, api_version)
	local data = {
		messages = { { role = "system", content = system_prompt }, { role = "user", content = prompt } },
		temperature = 0.7,
		stream = true,
...
function M.handle_azure_openai_spec_data(data_stream)
	if data_stream:match('"delta":') then
		local json = vim.json.decode(data_stream)
		if json.choices and json.choices[1] and json.choices[1].delta then
			local content = json.choices[1].delta.content
